...
public class ShopByCategory {
	...
	public void the_user_should_navigate_to_the_category_page_and_the_page_title_should_contain(String expectedTitle) {
		...
		try {
			...
			// -----------------------------------------------------------------
			// FIX 4: The original code caught AssertionError but never
			// rethrew it, so Cucumber marked every step as PASSED even when
			// the assertion failed. Re-throwing ensures the step is correctly
			// reported as FAILED in the Cucumber report.
			// -----------------------------------------------------------------
